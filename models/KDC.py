class KDC:
    ...
    def init_graph(self):
        ...
        jump_times es una lista con los tiempos en los que se produce un salto (para algún agente)
        G es el grafo que representa la adyacencia entre agentes
        '''
        jump_times = [0]
    ...
    def integrate(self):
        ...
        while self.jump_times[-1] < self.T:
            ...
            self.jump_times.append(self.jump_times[-1] + np.random.exponential(1 / (n_closed_edges / self.eps + n_open_edges * self.kappa))) 
            # Con un poco de fé, esto tiene la distribucion correcta
    ...
    def delta(theta):
        ...
        '''
        theta = theta % (2 * np.pi)
        deltas = KDC.angle_dif(theta[:, np.newaxis] - theta)
